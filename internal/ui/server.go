...
import (
	...
	webassets "agentwritegateway/web"
)
...
func New(app application.Gateway, identity IdentityVerifier) (*Server, error) {
...
func (s *Server) approvals(w http.ResponseWriter, r *http.Request) {
	...
	http.SetCookie(w, &http.Cookie{Name: "awg_csrf", Value: token, Path: "/ui/", HttpOnly: true, Secure: r.TLS != nil, SameSite: http.SameSiteStrictMode})
