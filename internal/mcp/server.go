...
func NewHTTP(app application.Gateway, resolver PrincipalResolver, explainer Explainer) *Server {
	return &Server{app: app, resolver: resolver, explainer: explainer}
}

func New(app application.Gateway, principal Principal, explainer Explainer) *Server {
...
func (s *Server) handle(ctx context.Context, message request) (any, *rpcError) {
	...
	case "initialize":
		...
		return map[string]any{"protocolVersion": ProtocolVersion, "capabilities": map[string]any{"tools": map[string]any{"listChanged": false}}, "serverInfo": map[string]string{"name": "agent-write-gateway", "version": "v1alpha1"}}, nil
	case "tools/list":
